...
boost::function<void (char*, char* , char*)> powerXCallback;
...
Device device;
unsigned char *DeviceBid = new unsigned char[17];
unsigned char *DeviceSip = new unsigned char[4];
/**
 *解析具体数据
 */
void parseDetail(unsigned char*data,size_t length){
   ...
   if (Frame_Type[0] == 0x02)
   {
    ...
    setSystemTimeWithGps(Time);
    if(powerXCallback){
        powerXCallback(powerData,tempData,gpsData);
    }
    //_hbt->httpXPower(powerData,tempData,gpsData);
   ...
   if (Frame_Type[0] == 0x08)
   {
      memcpy(DeviceBid, data+1, 17); 
      memcpy(DeviceSip, data+18, 4);
...
int UARTConfigData(unsigned char*ip,unsigned char*bid){
    ...
    memcpy(msg.Bid,bid,17);
    memcpy(msg.Server_IP,ip,4);
    // msg.Server_IP[0] = 47;
    // msg.Server_IP[1] = 89;
    // msg.Server_IP[2] = 51;
    // msg.Server_IP[3] = 239;
